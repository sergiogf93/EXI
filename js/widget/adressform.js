...
AddressForm.prototype.getBuffer = function() {
	if (this.buffer == null){
		this.buffer = {};
	}
...
AddressForm.prototype.load = function(buffer) {
	console.log(buffer)
	this.buffer = buffer;
	
	if (buffer != null){
		Ext.getCmp(this.id + "buffer_cardName").setValue(this.buffer.cardName);
...
AddressForm.prototype._getTopPanel = function() {
	return {
		xtype : 'container',
		layout : 'hbox',
		border : 0,
