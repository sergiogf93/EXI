...
SubtractionSaxsDataAdapter.prototype.getImage = function(subtractionId, imageType){
	...
	return connection.url + ('/{0}/proposal/{1}/saxs/subtraction/{2}/image/{3}'.format([ connection.token, connection.proposal, subtractionId, imageType]));
};

SubtractionSaxsDataAdapter.prototype.getZip = function(subtractionId){
	var connection = EXI.credentialManager.getConnections()[0];
	return connection.url + ('/{0}/proposal/{1}/saxs/subtraction/{2}/zip'.format([ connection.token, connection.proposal, subtractionId]));
};